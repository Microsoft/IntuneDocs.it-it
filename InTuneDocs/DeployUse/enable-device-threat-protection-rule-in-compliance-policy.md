...
Intune con la protezione dalle minacce per i dispositivi mobili Lookout offre la possibilità di rilevare le minacce per dispositivi mobili ed eseguire una valutazione dei rischi nel dispositivo.  
La regola dei criteri di conformità consente di usare questa valutazione dei rischi per determinare se il dispositivo è conforme ai criteri. È quindi possibile usare i criteri di accesso condizionale per consentire o bloccare Exchange, SharePoint e altri servizi in base alla conformità del dispositivo.

Per fare in modo che il rilevamento delle minacce Lookout MTP influenzi i criteri di conformità per il dispositivo:

1.  È necessario abilitare la regola **Protezione dalle minacce per il dispositivo** nei criteri di conformità.

2.  La pagina **Stato di Lookout** nella console di amministrazione Intune deve indicare **Attivo**. Vedere l'argomento [Abilitare la connessione a Lookout MTP nella console di amministrazione Intune](enable-lookout-mtp-connection-in-intune.md) per altri dettagli e istruzioni su come attivare l'integrazione di Lookout.


## Configurare la regola di protezione dalle minacce per il dispositivo
...
Nella console di amministrazione Intune, nella pagina dei criteri di conformità, passare a **Integrità del dispositivo** e abilitare la regola **Protezione dalle minacce per il dispositivo** tramite l'opzione di alternanza. Selezionare quindi il livello di minaccia massimo consentito tra uno dei seguenti:
...
* **Media**: il dispositivo viene valutato come conforme se le minacce presenti nel dispositivo sonio di livello basso o medio. Se viene rilevata la presenza di minacce di alto livello, il dispositivo viene determinato come non conforme.
* **Alta**: questo è il livello meno sicuro. Fondamentalmente consente tutti i livelli di minaccia ed è probabilmente utile solo se si usa questa soluzione esclusivamente per la creazione di report.
...
Se si creano criteri di accesso condizionale per Office 365 e altri servizi, viene presa in considerazione la valutazione di conformità precedente e per i dispositivi non conformi viene bloccato l'accesso fino alla risoluzione della condizione di minaccia.

È possibile visualizzare lo stato di conformità di un dispositivo nella pagina dei dispositivi della console di amministrazione Intune.
