...
Per i dispositivi che eseguono Windows 8.1 o Windows 10 Desktop o Mobile, è possibile importare un profilo di configurazione Wi-Fi precedentemente esportato in un file. Per informazioni dettagliate, vedere **Importare un profilo Wi-Fi** più avanti in questo argomento.
...
|**SSID (identificatore del set di servizi di rete)**|Specificare l'identificatore del set di servizi di rete (SSID) della rete wireless a cui i dispositivi devono connettersi. Per l'identificatore SSID viene fatta distinzione tra maiuscole e minuscole e non viene visualizzato dagli utenti.|
|**Connetti automaticamente quando la rete si trova nel campo del computer**|Selezionare questa opzione per consentire ai dispositivi di connettersi automaticamente alla rete wireless quando si trova nel campo del computer.|
|**Connetti quando la rete non sta trasmettendo il nome (SSID)**|selezionare questa opzione per consentire ai dispositivi di connettersi alla rete quando non è visibile nell'elenco di reti (perché è nascosta e non trasmette il proprio nome).|
