---
title: Guida per gli sviluppatori di Microsoft Intune App SDK per iOS | Microsoft Docs
description: Microsoft Intune App SDK per iOS consente di integrare i criteri di protezione delle app di Intune nell&quot;app iOS usando la gestione delle app mobili (MAM).
keywords: 
author: mtillman
manager: angrobe
ms.author: mtillman
...
> [!NOTE]
> È possibile leggere prima l'articolo [Introduzione a Microsoft Intune App SDK](intune-app-sdk-get-started.md), che spiega come preparare l'integrazione in ogni piattaforma supportata.
...
* **Intestazioni**: espongono le API di Intune App SDK. Se si usa un'API, è necessario includere il file di intestazione contenente l'API. I file di intestazione seguenti includono le chiamate alle funzioni API necessarie per abilitare le funzionalità di Intune App SDK:
...
1. **Opzione 1 (consigliata)**: collegare `IntuneMAM.framework` al progetto. Trascinare `IntuneMAM.framework` nell'elenco **Linked Frameworks and Libraries** (Framework e librerie collegati) di destinazione del progetto.

    > [!NOTE]
    > Se si usa il framework, prima di inviare l'app all'App Store, eliminare manualmente le architetture del simulatore dal framework universale. Per altri dettagli, vedere [Inviare l'app all'App Store](#Submit-your-app-to-the-App-Store).
...
2. **Opzione 2**: collegarsi alla libreria `libIntuneMAM.a`. Trascinare la libreria `libIntuneMAM.a` nell'elenco **Linked Frameworks and Libraries** (Framework e librerie collegati) di destinazione del progetto.
    ...
    > [!NOTE]
    > Se si intende rilasciare l'app nell'App Store, usare la versione di `libIntuneMAM.a` compilata per il rilascio anziché la versione di debug. La versione di rilascio si trova nella cartella **release** (rilascio). La versione di debug offre un output dettagliato utile per la risoluzione di problemi con Intune App SDK.
...
4. Aggiungere il bundle di risorse `IntuneMAMResources.bundle` al progetto trascinandolo in **Copy Bundle Resources** (Copia risorse bundle) in **Build Phases** (Compila fasi).
...
6. Abilitare la condivisione Keychain, se non è già abilitata, scegliendo **Capabilities** (Funzionalità) in ogni destinazione del progetto e abilitando l'opzione **Keychain Sharing** (Condivisione Keychain). La condivisione Keychain è necessaria per procedere con il passaggio successivo.
...
7. Dopo avere abilitato la condivisione Keychain, seguire questa procedura per creare un gruppo di accesso separato in cui Intune App SDK archivierà i dati. È possibile creare un gruppo di accesso a Keychain usando l'interfaccia utente o il file di diritti. Se si usa l'interfaccia utente per creare un gruppo di accesso a Keychain, seguire questa procedura:
    ...
    3. Aggiungere `com.microsoft.adalcache` ai gruppi di accesso esistenti.
      ...
      5. Se si usa il file di diritti per creare il gruppo di accesso a Keychain, è necessario anteporre `$(AppIdentifierPrefix)` al gruppo di accesso a Keychain nel file di diritti. Ad esempio:

            * `$(AppIdentifierPrefix)com.microsoft.intune.mam`
            * `$(AppIdentifierPrefix)com.microsoft.adalcache`

    > [!NOTE]
    > Un file di diritti è un file XML specifico per l'applicazione mobile. Consente di specificare autorizzazioni e funzionalità speciali nell'app iOS.
...
MAMPolicyRequired| Boolean| Specifica se verrà impedito l'avvio dell'app se l'app non include i criteri di protezione delle app di Intune. L'impostazione predefinita è NO. <br><br> Nota: le app non possono essere inviate all'App Store se MAMPolicyRequired è impostata su YES (Sì). | Facoltativo. |
MAMPolicyWarnAbsent | Boolean| Specifica se l'app avviserà l'utente durante l'avvio se l'app non include i criteri di protezione delle app di Intune. Si noti che le app non possono essere inviate all'archivio se per questa impostazione è specificato YES (Sì). | Facoltativa. |
...
In qualsiasi momento, ogni thread ha un'identità effettiva per le attività dell'interfaccia utente e le attività di file. Si tratta dell'identità usata per determinare i criteri da applicare, se presenti. Se l'identità non esiste o l'utente non è gestito, non viene applicato alcun criterio. I diagrammi seguenti illustrano come vengono determinate le identità effettive.
  ...
  ![Intune App SDK per iOS: Linked Frameworks and Libraries (Framework e librerie collegati)](../media/intune-app-sdk/ios-thread-identities.png)
