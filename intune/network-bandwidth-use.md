...
[Ottimizzazione recapito può essere impostata](delivery-optimization-settings.md) come parte dei profili di configurazione dei dispositivi.
...
> [!NOTE]
> È possibile usare Microsoft Intune per gestire i PC Windows [come dispositivi mobili con la soluzione di gestione dei dispositivi mobili (MDM)](windows-enroll.md) o come computer con il software client di Intune. Si consiglia ai clienti di [usare la soluzione di gestione MDM](windows-enroll.md) laddove possibile. Con questo tipo di gestione BranchCache non è supportato. Per altre informazioni, vedere [Confrontare la gestione dei PC Windows come computer o come dispositivi mobili](pc-management-comparison.md).
