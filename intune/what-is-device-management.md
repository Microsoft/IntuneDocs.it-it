...
description: Microsoft 365 Enterprise include Microsoft Intune. Vedere offerte da Intune per dispositivi mobili e gestione delle applicazioni per dispositivi mobili dell'organizzazione. Leggere gli scenari comuni e usare Intune per distribuire Microsoft 365 nell'ambiente in uso.
...
Un'attività chiave che spetta a qualsiasi amministratore è la protezione e messa in sicurezza delle risorse e dei dati di un'organizzazione. Questa attività è la *gestione dei dispositivi*. Spesso gli utenti dispongono di molti dispositivi che usano per aprire e condividere file personali, visitare siti Web e installare app e giochi. Questi stessi utenti sono anche i dipendenti e studenti. Desiderano usare i propri dispositivi per accedere al lavoro e risorse, ad esempio OneNote e di posta elettronica dell'istituto di istruzione. La gestione dei dispositivi consente alle organizzazioni di proteggere e mettere in sicurezza le risorse e i dati. 
...
- Supportare un ambiente di dispositivi mobili eterogeneogestire i dispositivi iOS, Android, Windows e macOS in modo sicuro
...
- **Microsoft 365** è un componente chiave per semplificare le attività IT più comuni. Nell'interfaccia di amministrazione di Microsoft 365, gli utenti di creare e gestire gruppi. È anche possibile accedere ad altri servizi, ad esempio Intune, Azure AD e così via. 

  Ad esempio, è possibile creare un gruppo di dispositivi iOS in Microsoft 365. Quindi, con Intune si possono distribuire i criteri tramite push al gruppo di dispositivi iOS che usano le funzionalità iOS, ad esempio l'accesso all'app store, l'uso di AirDrop, l'esecuzione di backup in iCloud, l'uso del filtro Web di Apple e altro ancora.
...
- **Windows Defender** include molte funzionalità di sicurezza per proteggere i dispositivi Windows 10. Ad esempio, usando Intune e Windows Defender insieme, è possibile: 
    ...
    - Uso [Windows Defender Advanced Threat Protection (ATP)](https://docs.microsoft.com/intune/advanced-threat-protection) per prevenire le violazioni della sicurezza nei dispositivi mobili. Inoltre, consentono di limitare l'impatto di una violazione della sicurezza tramite il blocco di un utente alle risorse aziendali.
...
Quando si configura Intune, è possibile scegliere di gestire i dispositivi esclusivamente nel portale di Azure oppure usando Intune e Microsoft 365 insieme. [Gestione dei dispositivi mobili la migrazione a Intune nel portale di Azure](https://www.microsoft.com/itshowcase/Article/Content/1042/Migrating-mobile-device-management-to-Intune-in-the-Azure-portal) è un case study Microsoft IT. In questo caso di Studio, vedere come Microsoft IT è stato scelto un approccio alla gestione moderna dei dispositivi e leggere le lezioni apprese.
...
Quando si è pronti per iniziare a usare una soluzione MDM o MAM, eseguire i diversi passaggi per configurare Intune, registrare i dispositivi e iniziare a creare i criteri, vedere Gestione dei dispositivi mobili per Microsoft 365. [Gestione dei dispositivi mobili per Microsoft 365](https://docs.microsoft.com/microsoft-365/enterprise/mobility-infrastructure) è anche un'ottima risorsa.
