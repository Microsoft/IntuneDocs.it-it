...
1. Accedere a https://portal.azure.com usando le credenziali di amministratore tenant di Intune e cercare le risorse di Intune.
2. Scaricare NDESConnectorSetup.exe da **Microsoft Intune** > **Configurazione del dispositivo** > **Autorità di certificazione** > **collegamento Scarica il connettore del certificato**
...
2. Creare un profilo certificato attendibile nel portale di amministrazione di Intune:
    ...
    b. Creare un profilo certificato attendibile da **Microsoft Intune** > **Configurazione del dispositivo** - **Profili** > **Crea profilo**.
...
2. Creare un profilo certificato PKCS da **Microsoft Intune** > **Configurazione del dispositivo - Profili** > **Crea profilo** > **Certificato PKCS**.
   ...
   > I parametri seguenti del profilo certificato PKCS devono essere configurati con valori specificati nella tabella seguente, come illustrato nello screenshot, per rilasciare certificati PKCS tramite Connettore di certificati di Intune dalla CA Symantec. 
    ...
    | --- | --- | --- |
    | Autorità di certificazione | pki-ws.symauth.com | Questo valore deve essere l'URL del servizio di base della CA Symantec senza barre finali.  Se non si è certi che questo sia l'URL del servizio di base corretto per la sottoscrizione della CA Symantec, contattare il supporto tecnico clienti Symantec. <br><br> Se questo URL non è corretto, Connettore di certificati di Intune non rilascerà certificati PKCS dalla CA Symantec.| 
