...
- **Invio di dati di diagnostica (solo Samsung Knox)**: scegliere **Blocca** per impedire all'utente di inviare dati di diagnostica dal dispositivo. **Non configurata** consente all'utente di inviare i dati.
...
- Se l'app Portale aziendale esegue la versione 1704 o versioni successive, è possibile applicare solo un PIN semplice. Le versioni di Android precedenti alla 5.0 non supportano questa impostazione. Nell'interfaccia di amministrazione di Microsoft Endpoint Manager non viene visualizzato alcun errore.
