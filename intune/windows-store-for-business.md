...
> [!NOTE]
> Se si usa più di uno strumento di gestione per assegnare le app di Windows Store per le aziende, in precedenza era possibile associare soltanto un'app a Windows Store per le aziende. È ora possibile associare più strumenti di gestione allo Store, ad esempio Intune e Configuration Manager.
...
Quando si assegna un'app di Windows Store per le aziende, viene usata una licenza per ogni utente che installa l'app. Se si usano tutte le licenze disponibili per un'app assegnata, non sarà possibile assegnare altre copie. Scegliere una delle azioni seguenti:
...
> [!Important]
> Le app assegnate sono disponibili solo per l'utente che ha originariamente registrato il dispositivo. Gli altri utenti non possono accedere all'app.
