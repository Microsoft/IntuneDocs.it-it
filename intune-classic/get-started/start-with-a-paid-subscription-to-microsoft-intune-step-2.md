...
4. Verrà visualizzata la finestra di dialogo **Verifica dominio** con i valori necessari per creare il record TXT nel provider di hosting DNS.
    ...
    > [!TIP]
    > Verificare di creare un alias DNS /intune-classic/deploy-use/set-up-windows-phone-management-with-microsoft-intune) mentre si apportano le modifiche nel provider di hosting DNS.
