---
title: Impostazioni dei criteri MAM per iOS
...
> [!IMPORTANT]
> Gran parte del contenuto di questa pagina non è aggiornato poiché è stata eseguita la migrazione di tutti i criteri di protezione delle app di Intune nel portale di Azure. Per altre informazioni, vedere i [criteri di protezione delle app Intune per iOS nel portale di Azure](https://docs.microsoft.com/intune/app-protection-policy-settings-ios).
...
| **Controlla di nuovo i requisiti di accesso dopo (minuti)** | Configurare le seguenti impostazioni: <ul><li>**Timeout**: numero di minuti prima che vengano ricontrollati i requisiti di accesso definiti in precedenza nei criteri. Ad esempio, un amministratore attiva il PIN nel criterio e l'utente deve immettere un PIN quando apre un'app MAM. Quando si usa questa impostazione, l'utente non dovrà immettere un PIN per alcuna app MAM per altri **30 minuti** (valore predefinito).<br><br>Il timeout per i requisiti di accesso viene misurato in termini di tempo di inattività tra tutte le applicazioni gestite tramite criteri.<br><br></li><li>**Periodo di prova offline**: numero di minuti per cui è consentita l'esecuzione di app MAM offline. Specificare il periodo (in minuti) prima che vengano ricontrollati i requisiti di accesso per l'app. Valore predefinito: **720** minuti (12 ore). Alla scadenza di questo periodo, l'app richiederà l'autenticazione utente per AAD per poter continuare l'esecuzione.</li></ul>| Timeout: 30 <br><br> Offline: 720 |
...
| **Richiedi una versione minima dell'SDK dei criteri di protezione app di Intune** | Scegliere **Sì** per richiedere una versione minima dell'SDK dei criteri di protezione delle app di Intune per l'app da usare. All'utente verrà impedito l'accesso se la versione dell'SDK dei criteri di protezione delle app di Intune dell'app non soddisfa il requisito. <br> <br> Per altre informazioni sull'SDK dei criteri di protezione delle app di Intune, vedere [Panoramica di Intune App SDK](/intune/app-sdk) <br><br> | No |
